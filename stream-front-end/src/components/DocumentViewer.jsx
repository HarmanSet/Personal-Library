...
//   const documentUrl = `http://localhost:8080/api/v1/videos/stream/${document.videoId}`;
...
const DocumentViewer = ({ document }) => {
  ...
  const documentUrl = `http://localhost:8080/api/v1/videos/stream/${document.videoId}`; // Adjust this URL accordingly
  ...
  return (
    <Card className="bg-white rounded-lg shadow-md overflow-hidden">
      <div className="p-4 border-b">
        ...
        <div className="flex items-center text-sm text-gray-500 mt-1">
          {document.filename && (
            <span className="ml-4 bg-blue-100 text-blue-800 text-xs font-semibold px-2.5 py-0.5 rounded">{'PDF'}</span>
          )}
        </div>
      </div>
